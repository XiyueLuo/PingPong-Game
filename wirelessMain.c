...
void Initialize(){
	...
	//Set the Joysticker-Connect to digital io
	DDRD &= ~(1<<DDD4);//PD1 for up button
	DDRD &= ~(1<<DDD7);//PD4 for down button
...
uint8_t paddle(uint8_t player_y0){
	LCD_drawBlock(0,player_y0,4,player_y0+30,rgb565(255,255,255)); //invisible old one
	int player_speed=0;
	if(ADC <500){ //go up
	...
	LCD_drawBlock(0, player_y0,4, player_y0+30,rgb565(0,255,0)); // show the paddle
	return player_y0;
}

uint8_t computer(uint8_t computer_y0){
	int computer_speed=0;
	LCD_drawBlock(155, computer_y0, 159, computer_y0+45, rgb565(255,255,255));//invisible old one
	if(PIND&(1<<PIND4)){ // go up
	...
	if( (!(PIND&(1<<PIND4))) && (!(PIND&(1<<PIND7))) ){//stay
		computer_speed = 0;
	}
	computer_y0 = computer_y0 + computer_speed;
	LCD_drawBlock(155, computer_y0, 159, computer_y0+45, rgb565(255,0,0));//show the paddle
	return computer_y0;
}

struct Position ball(struct Position p){
	LCD_drawCircle(p.ball_x, p.ball_y, 5, rgb565(255,255,255)); //invisible
	...
	LCD_drawCircle(p.ball_x, p.ball_y, 5, rgb565(255,0,255));
	return p;
}
...
int main(void)
{
    ...
    while (1) 
    {
		sprintf(String,"ADC is %d \n", PIND&(1<<PIND4));
		UART_putstring(String);
		LCD_drawString(30,0,"Welcome to Pong!",rgb565(0,0,0),rgb565(255,255,255));
		LCD_drawString(50,10,"Round",rgb565(0,0,0),rgb565(255,255,255));
		LCD_drawChar(80,10,0x30+round_player,rgb565(0,0,0),rgb565(255,255,255));
		LCD_drawChar(87,10,0x2d,rgb565(0,0,0),rgb565(255,255,255));
		LCD_drawChar(94,10,0x30+round_computer,rgb565(0,0,0),rgb565(255,255,255));
		
		LCD_drawString(50,20,"Score",rgb565(0,0,0),rgb565(255,255,255));
		LCD_drawChar(80,20,0x30+score_player,rgb565(0,0,0),rgb565(255,255,255));
		LCD_drawChar(87,20,0x2d,rgb565(0,0,0),rgb565(255,255,255));
		LCD_drawChar(94,20,0x30+score_computer,rgb565(0,0,0),rgb565(255,255,255));
		player_y=paddle(player_y);
		computer_y=computer(computer_y);
		ball_position=ball(ball_position);
    }
}
